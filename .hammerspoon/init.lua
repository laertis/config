...
local function newWindow(appName, menuPath)
    local app = hs.application.get(appName)
    ...
    -- Attempt to create a new window via the specified menu path
    if not app:selectMenuItem(menuPath) then
        -- Fallback: just focus the app if menu item doesn't exist
        hs.application.launchOrFocus(appName)
    end
end

hs.hotkey.bind({"shift", "command"}, "e", function()
	newWindow("Finder", {"File", "New Finder Window"})
end)

hs.hotkey.bind({"shift", "alt"}, "2", function()
	newWindow("iTerm", {"Shell", "New Window"})
end)
